...
void CantTiemp (llamadas *f,int lineas,int &cant1,int &cant2,int &cant3,float &tiemp1,float &tiemp2,float &tiemp3);
void Monto (llamadas *f,int lineas,float &m1,float &m2,float &m3,float tiemp1,float tiemp2,float tiemp3);
...
int main(void){
    ...
    if(error){
         ...
         CantTiemp(datos,cantidad,cantidad1,cantidad2,cantidad3,tiempo1,tiempo2,tiempo3);
         
         cout<<"Cantidad de llamadas '1': "<<cantidad1<<"\tduracion: "<<tiempo1;
         cout<<"\tCantidad de llamadas '2': "<<cantidad2<<"\tduracion: "<<tiempo2;
         cout<<"\tCantidad de llamadas '3': "<<cantidad3<<"\tduracion: "<<tiempo3<<endl;
         
         Monto(datos,cantidad,monto1,monto2,monto3,tiempo1,tiempo2,tiempo3);
...
llamadas *LeerArchivo (int &error,int &lineas){
	...
	lineas++;
	aux->horas=tmp;
	entrada >> aux->minutos >> aux->segundos>> aux->duracion >> aux-> codigo>>tmp;
      }
    }
    entrada.close();
  }
